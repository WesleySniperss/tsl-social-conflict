...
- **Dropped the Rattled native-alias entirely** (it was the thing forcing a 2-status effect). Rattled now registers as our own `⚔ Rattled` single-status entry with its own `a5eChanges` — no longer piggybacks on A5E's native Rattled. Trade-off accepted for consistent HUD behaviour; twin-detection/`nativeAlias` code removed from the registration loop (the `alias &&` guards in getActiveCondition/removeCondition are now dead but harmless).
